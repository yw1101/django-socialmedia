...
class NewsFeedApiTests(TestCase):
    ...
    def test_pagination(self):
        ...
        for i in range(page_size * 2):
            ...
            newsfeed = self.create_newsfeed(user=self.kellynim, tweet=tweet)
        ...
        response = self.kellynim_client.get(
            NEWSFEEDS_URL,
        ...
        tweet = self.create_tweet(followed_user)
        new_newsfeed = self.create_newsfeed(user=self.kellynim, tweet=tweet)
    ...
    def test_user_cache(self):
        profile = self.talenti.profile
        profile.nickname = 'kiehls'
        profile.save()

        self.assertEqual(self.kellynim.username, 'kellynim')
        self.create_newsfeed(self.talenti, self.create_tweet(self.kellynim))
        self.create_newsfeed(self.talenti, self.create_tweet(self.talenti))

        response = self.talenti_client.get(NEWSFEEDS_URL)
        results = response.data['results']
        self.assertEqual(results[0]['tweet']['user']['username'], 'talenti')
        self.assertEqual(results[0]['tweet']['user']['nickname'], 'kiehls')
        self.assertEqual(results[1]['tweet']['user']['username'], 'kellynim')

        self.kellynim.username = 'Charlotte'
        self.kellynim.save()
        profile.nickname = 'Tilbury'
        profile.save()
    ...
    def test_redis_list_limit(self):
        ...
        for i in range(list_limit + page_size):
            ...
            feed = self.create_newsfeed(self.kellynim, tweet)
            newsfeeds.append(feed)
        newsfeeds = newsfeeds[::-1]

        # only cached list_limit objects
        cached_newsfeeds = NewsFeedService.get_cached_newsfeeds(self.kellynim.id)
        self.assertEqual(len(cached_newsfeeds), list_limit)
        queryset = NewsFeed.objects.filter(user=self.kellynim)
        self.assertEqual(queryset.count(), list_limit + page_size)

        results = self._paginate_to_get_newsfeeds(self.kellynim_client)
        ...
        self.create_friendship(self.kellynim, self.talenti)
        new_tweet = self.create_tweet(self.talenti, 'a new tweet')
