...
class CommentApiTests(TestCase):
    ...
    def test_comments_count_with_cache(self):
        ...
        response = self.kellynim_client.get(tweet_url)
        ...
        comment_data = self.talenti_client.post(COMMENT_URL, data).data
        ...
        response = self.talenti_client.put(comment_url, {'content': 'updated'})
        self.assertEqual(response.status_code, 200)
        response = self.talenti_client.get(tweet_url)
        self.assertEqual(response.data['comments_count'], 3)
        self.tweet.refresh_from_db()
        ...
        response = self.talenti_client.delete(comment_url)
        self.assertEqual(response.status_code, 200)
        response = self.kellynim_client.get(tweet_url)
